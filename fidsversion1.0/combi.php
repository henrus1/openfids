...
echo '<colgroup>
	...
	while (list($airlinecode,$flightno,$departs,$departstime,$airport,$registration,$slottime,$edt,$bay,$gate,$aircraft,$checkin,$status)=mysql_fetch_row($dtable)){
	




	$destinationname = mysql_result(mysql_query("SELECT `airportname` FROM `airports` WHERE `airportcode` = '".$airport."'"),0);
	$airlinename = mysql_result(mysql_query("SELECT `airlinename` FROM `airlines` WHERE `airlinecode`='".$airlinecode."'"),0);
...
if(!$atable){
	...
	while (list($airlinecode,$flightno,$arrives,$arrivestime,$airport,$registration,$slottime,$eta,$bay,$gate,$aircraft,$belt,$type,$status,$staffmsg)=mysql_fetch_row($atable)){
